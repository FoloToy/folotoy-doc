...
| stt_type |  sound-to-text 引擎类型，可选值：openai-whisper, azure-whisper, azure-tts   | openai-whisper | 
| stt_config |  sound-to-text 引擎具体配置，不同类型的引擎配置参数不同   | 无 | 
| llm_type |  large-language-model 类型，可选值： openai，azure-openai, 也可以通过 [one-api](https://github.com/songquanpeng/one-api) 来中转大模型接口为 openai-like 从而支持更多的大模型 | openai | 
| llm_config |  大模型的配置  | 无 | 
