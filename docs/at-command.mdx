...
### Modify voice threshold in continuous dialogue mode:
    ...
    at+config=common,voice,<threshold> // default to 60dB, with a range of values ranging from 50dB to 80dB

- Example: Modify voice threshold to 60dB
...
### Wake up mode configuration (currently only supported in the Octopus MiniPro version)
    ...
    at+config=common,wake_up_mode,<value> // 0-disable，1-enable
